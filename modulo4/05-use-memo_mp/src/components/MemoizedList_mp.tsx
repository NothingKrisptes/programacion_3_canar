...
const INITIAL_TASKS: Tarea[] = [
  { id: 1, text: 'Revisar niveles de aceite y frenos', completed: false },
  { id: 2, text: 'Verificar presión de llantas',       completed: true  },
  { id: 3, text: 'Limpiar interior del vehículo',       completed: false },
  { id: 4, text: 'Revisar luces y espejos',             completed: false },
  { id: 5, text: 'Entregar vehículo al cliente',        completed: false },
]
...
export default function MemoizedList_mp() {
  const [tasks,   setTasks]   = useState<Tarea[]>(INITIAL_TASKS)
  const [contador, setContador] = useState(0)

  // useCallback: onToggle y onDelete tienen referencia estable
  // TaskRow no re-renderiza cuando solo cambia `contador`
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', maxWidth: 520, margin: '0 auto', padding: 24 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Checklist de Inspección</h2>
      ...
      <p style={{ marginTop: 16, fontSize: 12, color: '#aaa' }}>
        ...
        {' '}(debería crecer solo al hacer toggle o delete, no al pulsar el contador)
      </p>
    </div>
  )
}
